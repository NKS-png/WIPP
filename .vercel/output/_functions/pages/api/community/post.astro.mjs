...
const POST = async ({ request, cookies }) => {
  try {
    ...
    if (imageFile && imageFile.size > 0) {
      ...
      const { data: uploadData, error: uploadError } = await supabase.storage.from("projects").upload(filePath, imageFile, { cacheControl: "3600", upsert: false });
      if (uploadError) {
    ...
    if (type === "portfolio") {
      ...
      const { data: userProfile } = await supabase.from("profiles").select("sparks_balance").eq("id", sessionData.session.user.id).single();
      console.log("User sparks balance:", userProfile?.sparks_balance);
      if (!userProfile || (userProfile.sparks_balance || 0) < 1) {
        await supabase.from("profiles").update({ sparks_balance: 10 }).eq("id", sessionData.session.user.id);
        console.log("Added sparks to user for testing");
      }
      const work_stage = formData.get("work_stage");
      const intent = formData.get("intent");
      const feedback_goals = JSON.parse(formData.get("feedback_goals") || "[]");
